  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row gap-6">
      ...
      {sortedImages.length > 1 && (
        <div className="flex lg:flex-col gap-4 overflow-x-auto lg:overflow-y-auto lg:w-24 flex-shrink-0 hide-scrollbar pb-4 lg:pb-0">
          {sortedImages.map((img) => (
            <button
              key={img.id}
              onClick={() => setActiveImage(img.url)}
              className={`relative aspect-[3/4] w-20 lg:w-full flex-shrink-0 bg-gray-100 rounded-2xl overflow-hidden transition-all duration-300 ${
