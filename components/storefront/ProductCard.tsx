...
export function ProductCard({ product }: { product: any }) {
  ...
  return (
    <Link href={`/product/${product.slug}`} className="group block bg-white p-3 rounded-[2rem] shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300">
      {/* Image Container */}
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-gray-100 rounded-[1.5rem] mb-4">
      ...
      <div className="px-2 pb-1 text-center">
        {/* Category/Brand */}
        <div className="text-[10px] text-[#FF7A00] font-bold mb-2 tracking-[0.2em] uppercase">
          {product.categories?.name || 'Streetwear'}
        </div>
        
        {/* Title */}
        <h3 className="text-base font-bold text-gray-900 leading-snug mb-2 truncate">
          {product.name}
        </h3>
        ...
        <div className="mb-4">
          {product.sale_price ? (
            <div className="flex items-center justify-center space-x-2">
              <span className="text-[15px] font-bold text-gray-900">₹{product.sale_price.toFixed(2)}</span>
        ...
        {/* Button */}
        <div className="w-full bg-[#1C1C1C] text-white text-center py-4 rounded-full text-[11px] sm:text-xs font-black uppercase tracking-widest hover:bg-[#FF7A00] active:scale-95 transition-all duration-300">
          Buy Now
        </div>
      </div>
    </Link>
